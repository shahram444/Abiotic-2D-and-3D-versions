 ...
 * STEP 3:  D_lattice = (1/4) × (τ_ADE - 0.5)       [ADE diffusivity, D3Q7]
...
struct SimulationResult {
    ...
    SimulationResult() : success(false), nx(0), ny(0), nz(0), dx(0), porosity(0),
    ...
    T tau_NS, T tau_ADE, T nu_NS, T D_lattice, T L_domain)
{
    ...
    file << "Geometry,nx,ny,nz,dx_m,dt_s,Porosity,Permeability_lattice,Permeability_m2,"
...
int main(int argc, char** argv) {
    ...
    for (size_t fileIdx = 0; fileIdx < datFiles.size(); ++fileIdx) {
        ...
        try {
            ...
            // ════════════════════════════════════════════════════════════════
            // STEP 2-3: NS AND ADE PARAMETERS
            // ════════════════════════════════════════════════════════════════
            T nsLatticeTau = tau;
            ...
            // ════════════════════════════════════════════════════════════════
            // STEP 4-5: FIRST NS SIMULATION AND PERMEABILITY
            // ════════════════════════════════════════════════════════════════
            T PoreMaxUx = 0, DarcyOutletUx = 0;
            ...
            pcout << "  Permeability: " << permeability * dx * dx << " m²\n";

            // ════════════════════════════════════════════════════════════════
            // STEP 6-8: TARGET VELOCITY AND SECOND NS
            // ════════════════════════════════════════════════════════════════
            ...
            // ════════════════════════════════════════════════════════════════
            // STEP 9-10: EXTRACT VELOCITIES AND STABILITY CHECK
            // ════════════════════════════════════════════════════════════════
            PoreMaxUx = computeMax(*computeVelocityComponent(nsLattice, Box3D(1, nx - 2, 0, ny - 1, 0, nz - 1), 0));
            DarcyOutletUx = computeAverage(*computeVelocityComponent(nsLattice, Box3D(nx - 2, nx - 2, 0, ny - 1, 0, nz - 1), 0));
            ...
            if (!stability.Ma_ok) {
                throw std::runtime_error("Mach number too high (Ma >= 1.0). Flow divergence.");
            }
            
            if (!stability.tau_NS_ok) {
                throw std::runtime_error("tau_NS out of stable range.");
            }
            ...
            pcout << "  u_Darcy: " << u_Darcy_physical << " m/s\n";
            ...
            for (; iT < ade_maxiTer; ++iT) {
                ...
                if (ade_VTK_iTer > 0 && iT % ade_VTK_iTer == 0 && iT > 0) {
                    ...
                    pcout << "  iT=" << iT << " | PV=" << std::fixed << std::setprecision(2) << pore_vol 
